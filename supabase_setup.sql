...
-- Bookings: 預約記錄 (移至 reviews 之前，因為 reviews 引用它)
...
CREATE TABLE public.reviews (
  ...
  beautician_id uuid REFERENCES public.beauticians(id) ON DELETE CASCADE,
  user_id uuid REFERENCES auth.users(id) ON DELETE SET NULL, -- 改回 auth.users 避免 Profile 未同步導致錯誤
  customer_name text, -- 備份姓名
...
BEGIN
  ...
  (v_yaya_id, '熱石經絡按摩', '結合溫熱礦石，促進體體循環，深度緩解疲勞。', 2800, 90);
END $$;
